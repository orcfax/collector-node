...
try:
    ...
    # Disable CNT import to focus on CEX batching.
    #
...
async def fetch_cex_data(feeds: str) -> dict:
    ...
    logger.debug("fetching cex feeds using goder: %s ('%s')", config.GOFER, len(feeds))
    stdout = []
    try:
...
async def xfetch_cex_feeds(feeds: list[str]) -> AsyncGenerator:
    ...
    for feed in random.sample(feeds, len(feeds)):
        ...
        res.appennd(await fetch_cex_data(feed=feed))
        if not res:
...
async def fetch_cex_feeds(feeds: list[str]) -> AsyncGenerator:
    ...
    if len(res.keys()) < len(feeds):
        logger.error("cannot retrieve data for: '%s'", list(set(feeds).difference(["1", "2", "3"])))
    return res
...
async def send_to_ws(validator_websocket, data_to_send: dict):
    ...
    logger.info(
        "sending message containing; '%s'", list(data_to_send.keys()),
    )
